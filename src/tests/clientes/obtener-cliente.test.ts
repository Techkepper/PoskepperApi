...
describe('ClienteController - obtenerCliente', () => {
  ...
  /**
   * Prueba: debería devolver un error 400 si la identificación ya está en uso.
   */
  it('debería devolver 400 si la identificación ya está en uso', async () => {
    const req = {
      params: {
        id: '1',
      },
    ...
    ;(clienteRepository.existeIdCliente as jest.Mock).mockResolvedValueOnce(
      true,
    )
    ;(clienteRepository.existeCedulaCliente as jest.Mock).mockResolvedValueOnce(
      true,
    )
   ...
   */
  it('debería devolver 400 si el correo electrónico ya está en uso', async () => {
    const req = {
      params: {
        id: '1',
      },
    ...
    ;(clienteRepository.existeCorreo as jest.Mock).mockResolvedValueOnce(true)
  ...
  it('debería devolver 200 indicando que el cliente fue actualizado correctamente', async () => {
    const req = {
      ...
      body: {
        cedula: '1234567890',
        correoElectronico: 'newemail@example.com',
      },
    } as unknown as Request
    ...
    ;(clienteRepository.existeCorreo as jest.Mock).mockResolvedValueOnce(false)
    ;(clienteRepository.actualizar as jest.Mock).mockResolvedValueOnce(true)
  ...
  /**
   * Prueba: debería devolver un error 500 si hay problemas con el servidor.
   */
  it('debería devolver 500 si hay problemas con el servidor', async () => {
    const req = {
      params: {
        id: '1',
      },
    ...
    ;(clienteRepository.actualizar as jest.Mock).mockRejectedValue(
